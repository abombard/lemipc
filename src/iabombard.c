...
int		easytarget(t_pos *target, t_pos *ppos, t_parr *ally, t_parr *enemy)
{
	t_pos	a;
	t_pos	e;
	int		i;
...
void	iabombard(t_context *context)
{
	...
	ally.arr = find_all(context->map, &context->player, &isally, &ally.total);
	enemy.arr = find_all(context->map, &context->player, &isenemy, &enemy.total);
	...
	if (recv_target(context->mqid, context->player.id, action, &target))
		;
	else if (enemy.total && (ally.close == ally.total || ally.close > enemy.close))
	{
	...
	{
		target.x = context->player.pos.x + (context->player.pos.x - enemy.arr[0].x) % 1;
		target.y = context->player.pos.y + (context->player.pos.y - enemy.arr[0].y) % 1;
	}
